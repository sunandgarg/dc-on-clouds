-- Recover 200 course records left in failed state by the earlier AI cleaner.
...
SET
  ...
  description = p.clean_name || ' is a ' || lower(p.clean_level) || ' learning pathway in ' || lower(p.clean_category) || '. Compare the current curriculum, eligibility, recognition, assessment and practical exposure across institutions.',
  ...
  about_content = '<h2>About ' || p.clean_name || '</h2><p>' || p.clean_name || ' develops foundational and applied understanding in ' || lower(p.clean_category) || '. Programme structure and learning format vary by institution.</p>',
  ...
  subjects_content = '<h2>Subjects and structure</h2><p>Use the latest university syllabus for exact semester subjects, credits, electives, projects and practical work.</p>',
