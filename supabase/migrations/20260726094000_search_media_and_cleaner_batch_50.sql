-- Prefer real media assets when legacy rows store empty strings, and allow
-- larger bounded cleaning runs so exams/courses can be processed in 50-item passes.
...
AS $$
  ...
  candidates AS (
    SELECT 'College'::text AS entity_type,
           ...
           COALESCE(NULLIF(c.logo, ''), '')::text AS image_url
    ...
    SELECT 'Exam',
           ...
           COALESCE(NULLIF(e.logo, ''), NULLIF(e.image, ''), '')::text
...
BEGIN
  ...
  VALUES (
    _entity_types,
    greatest(1, least(coalesce(_batch_size, 5), 50)),
    _max_records,
