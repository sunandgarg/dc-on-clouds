-- Final pass over the normal pending course queue.
...
SET
  ...
  description = p.clean_name || ' is a ' || lower(p.clean_level) || ' learning pathway in ' || lower(p.clean_category) || '. Compare curriculum, eligibility, assessment, recognition and practical exposure across institutions before applying.',
  ...
  about_content = '<h2>About ' || p.clean_name || '</h2><p>' || p.clean_name || ' builds foundational and applied understanding in ' || lower(p.clean_category) || '. The exact curriculum and learning format vary by institution.</p>',
  scope_content = '<h2>Career and higher-study scope</h2><p>Outcomes depend on skills, projects, internships, location and institution. Compare realistic roles, certifications, higher-study routes and programme-level outcomes.</p>',
  subjects_content = '<h2>Subjects and structure</h2><p>Use the latest university syllabus for semester subjects, credits, practical work, projects and electives.</p>',
  admission_process = '<h2>Admission checklist</h2><ol><li>Confirm programme eligibility.</li><li>Check merit or entrance-test requirements.</li><li>Compare recognised institutions and current curricula.</li><li>Use authorised application channels.</li><li>Follow counselling or interview updates.</li></ol>',
  placements_content = '<h2>Placements and outcomes</h2><p>Compare median outcomes, participating students, roles, internships and recruiter diversity instead of relying only on the highest package.</p>',
