...
const input = process.argv[2] || "data/college-official-enrichment/live-next-100-batch-013.json";
const migration = process.argv[3] || "supabase/migrations/20260809200500_college_humanized_content_batch_013_100.sql";
const report = process.argv[4] || "reports/college-humanized-content-batch-013.md";
const researchedAt = process.argv[5] || "2026-08-09";
const batch = "college-humanized-content-batch-013-100";
...
const buildSections = (record, index) => {
  ...
  const relatedCourses = verifiedCourses && Array.isArray(record.related_courses) ? record.related_courses.map(cleanText).filter(Boolean) : [];
  const courseCount = verifiedCourses ? Number(record.courses_count || relatedCourses.length || 0) : 0;
  ...
  const courseFee = verifiedCourses
    ? html(
        h("Courses And Fees"),
...
if (!Array.isArray(rows) || rows.length !== 100) {
  errors.push(`Expected 100 records, got ${Array.isArray(rows) ? rows.length : "invalid input"}`);
}
...
rows.forEach((record, index) => {
  ...
  const sourcePayload = {
    ...
    course_policy: s.verifiedCourses ? "official_courses_preserved_from_existing_verified_mapping" : "courses_blank_until_official_catalogue_verified",
    fee_policy: s.verifiedFees ? "verified_fee_fields_preserved" : "fees_blank_until_programme_year_category_and_quota_are_verified"
  };
...
SET
  ...
  course_fee_content = CASE WHEN ${s.verifiedCourses ? "true" : "false"} THEN ${sql(s.courseFee)} ELSE 'Check the official college website for current courses and fees.' END,
  related_courses = CASE WHEN ${s.verifiedCourses ? "true" : "false"} THEN related_courses ELSE ARRAY[]::text[] END,
  courses_count = CASE WHEN ${s.verifiedCourses ? "true" : "false"} THEN GREATEST(COALESCE(courses_count, 0), ${s.courseCount}) ELSE 0 END,
  ...
  fees = CASE WHEN ${s.verifiedFees ? "true" : "false"} THEN fees ELSE '' END,
  ...
  data_clean_audit_note = ${sql(`${batch}; expanded rendered college content; official-only course policy enforced; fees preserved only when official_fees_verified is true; no long dashes; imported batch marker text removed from public fields.`)},
...
WHERE college_slug = ${sql(cleanSlug)}
  AND NOT (SELECT COALESCE(official_fees_verified, false) FROM public.colleges WHERE slug = ${sql(cleanSlug)});
`);

  reportRows.push(`| ${index + 1} | ${s.name.replace(/\|/g, "/")} | ${cleanSlug} | ${s.wordCount} | ${s.verifiedCourses ? "preserved" : "blank"} | ${s.verifiedFees ? "preserved" : "blank"} | ${s.sourceStatus} |`);
});
...
-- Generated ${researchedAt}. Next 100 college source-aware content expansion.
-- Course names are preserved only when production already marks official_courses_verified = true.
-- Fees are blank unless production already marks official_fees_verified = true.
BEGIN;
${blocks.join("\n")}
...
Policy:
- Courses: official verified only. If not verified, public course table is cleared/blanked.
- Fees: preserved only when official_fees_verified is true. Otherwise blank.
